...
EmberEui64 *emberMaxExtendedID(void)
{
	return (EmberEui64*)emMacExtendedId;
}
...
// network-management commission <preferred channel:1> <fallback channel mask:4> <network id:0--16> <ula prefix> <extended pan id:8> <key:16> [<pan id:2> [<key sequence:4>]]
void networkManagementCommissionCommand2(uint8_t preferredChannel, uint32_t fallbackChannelMask, uint8_t *networkId, uint8_t *strUlaPrefix,
										uint8_t *bigEndianExtendedPanId, uint8_t *contents, uint16_t panId, uint32_t keySequence)
{


  uint8_t networkIdLength = (uint8_t)strlen(networkId);
  if (networkIdLength > EMBER_NETWORK_ID_SIZE) {
    emberAfAppPrintln("%p: %p", "ERR", "invalid network id");
    return;
  }
  ...
  uint8_t prefixBits;
  EmberIpv6Prefix ulaPrefix;
  if (emberIpv6StringToPrefix(strUlaPrefix, &dst, &prefixBits) && prefixBits == 64) {
    MEMCOPY(&ulaPrefix, &dst, sizeof(EmberIpv6Prefix));
  } else {
    emberAfAppPrintln("%p: %p", "ERR", "invalid ula prefix");
    return;
  ...
  uint8_t extendedPanId[EXTENDED_PAN_ID_SIZE];
  if (strlen(bigEndianExtendedPanId)<=EXTENDED_PAN_ID_SIZE){
		emberReverseMemCopy(extendedPanId, bigEndianExtendedPanId, EXTENDED_PAN_ID_SIZE);
	} else {
    emberAfAppPrintln("%p: %p %d!=%d", "ERR", "bigEndianExtendedPanId", strlen(bigEndianExtendedPanId), EXTENDED_PAN_ID_SIZE);
    return;
  }


  if (strlen(contents) != EMBER_ENCRYPTION_KEY_SIZE) {
    emberAfAppPrintln("%p: %p", "ERR", "invalid key");
    return;
  }
  EmberKeyData key;
